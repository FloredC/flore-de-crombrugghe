...
// shape (radius-60 -- previously silently invalid, that key wasn't exposed in
// tailwind.config.js's borderRadius, so the class did nothing), white bg,
// border-b navbar-border, drop-shadow 0/0/5/0 black 25% (same shadow as the
// popover), 58x58 circular avatar/home link with a visible thin ring per the
// Figma screenshot (ic-home.svg -- the circular map crop Flore exported,
// resolving what that asset was for), 40px gap between nav links, 16px bold
// link text ("Mobile/body-bold" per Figma even on this desktop variant),
// Contact rendered as the existing secondary button chrome, not a plain link.
      ...
      >
        {isHome ? (
          <a href="#hero" aria-label="Back to the map">
            <img src={homeIcon} alt="" width={58} height={58} className="rounded-full border border-border-grey" />
          </a>
        ) : (
