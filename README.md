...
| `npm run copy <slug>` | Writes `copy/<slug>.md` — everything a visitor reads on that case study, and nothing else. For reviewing writing without scrolling a 1000-line module that is half provenance comments. `all` does every page; `--stdout` prints instead. **The output is committed**, so a copy change shows up in a diff as prose rather than as JavaScript — re-run `npm run copy all` and commit it alongside any content edit. See [`scripts/copy.mjs`](scripts/copy.mjs). |
