...
| `npm run copy <slug>` | Writes `copy/<slug>.md` — everything a visitor reads on that case study, and nothing else. For reviewing writing without scrolling a 1000-line module that is half provenance comments. `all` does every page; `-- --stdout` prints instead, `-- --dashes` lists every em dash with the block it sits in (rule 8). Each run reports the em-dash count per page, so "only exceptionally" is a number rather than an impression. **The output is committed**, so a copy change shows up in a diff as prose rather than as JavaScript — re-run `npm run copy all` and commit it alongside any content edit. See [`scripts/copy.mjs`](scripts/copy.mjs). |
